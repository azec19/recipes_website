...
export async function updateUser(req, res) {
    try {
            const newUser = await userService.updateUser(req.body.id, req.body.email, req.body.Name);
            res.status(201).json(newUser);
    } catch (error) {
        res.status(400).json({ message: error.message });
    }
...
const userController = { getUserById, createUser, getAllUser, deleteUser, updateUser };
